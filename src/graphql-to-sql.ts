...
export const DERIVED_FROM_DIRECTIVE =
...
const getBaseNode = (node: TypeNode): NamedTypeNode => {
  switch (node.kind) {
...
export const generateSQLSchema = async ({ source }: { source: string }) => {
  let sqlSchema: Array<{
    tableName: string;
    columns: Array<ColumnsAST> | undefined;
  }> = [];
  let insertStatements: Array<string> = [];

  const insertStatement = (tableName: string, value: string) =>
    ...
    ObjectTypeDefinition(node) {
      ...
      const columns = node.fields
        ...
        .map(({ name, type, directives }) => {
          ...
          const namedNode = getBaseNode(type);
          ...
          const listType = namedNode
            ...
            ? type.type.kind === Kind.LIST_TYPE
              ...
              : null
            : null;
          const listNode = listType ? getBaseNode(listType) : null;

          const namedType = namedNode.name.value
  ...
  const schema = sqlSchema.map(({ tableName, columns }) => {
    const cols = columns?.map(
      ({ name, type, constraint, relationTable }, i) => {
